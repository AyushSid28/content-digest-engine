...
def _stream_sse(input: str, model: str, provider: str, theme: str):
    ...
    yield f"data: {{\"status\": \"detected\", \"type\": \"{input_type}\"}}\n\n"
    ...
    try:
        if input_type == "url":
            ...
            yield f"data: {{\"status\": \"metadata\", \"title\": \"{metadata['title']}\"}}\n\n"
            ...
            yield f"data: {{\"status\": \"metadata\", \"name\": \"{metadata['name']}\"}}\n\n"
            readme = fetch_readme(owner, repo)
            tree = fetch_tree(owner, repo, metadata['default_branch'])
            content = build_github_context(metadata, readme, tree)

        else:
            yield f"data: {{\"error\": \"Unsupported input type for browser: {input_type}\"}}\n\n"
            return
    ...
    yield f"data: {{\"status\": \"summarising\"}}\n\n"
    ...
    if not api_key and not all_keys:
        yield "data: {\"error\": \"No API keys configured\"}\n\n"
        return
