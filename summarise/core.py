...
from .output import render_markdown, render_json, console
...
def summarise_pipeline(
    ...
    json_mode: bool = False, theme: str = "default",
):
    ...
    try:
        if input_type == "url":
            ...
            with progress:
                ...
                progress.update(task, description="Fetching transcript...")
            ...
            with progress:
                task = progress.add_task("Fetching repo data...", total=None)
                ...
                progress.update(task, description="Fetching README & tree...")
            ...
            with progress:
                task = progress.add_task("Downloading podcast...", total=None)
    ...
    def _summarise(text: str):
        if api_key:
            return stream_response(text, api_key, model, provider_name=provider, system_prompt=theme_prompt)
        return stream_response_with_fallback(text, all_keys, model=None, preferred=provider, system_prompt=theme_prompt)

    if json_mode:
        console.print(f"[bold]Summarising with {provider}/{model} (JSON mode)...[/bold]\n")
        try:
            full = ""
            for token in _summarise(chunks[0] if len(chunks) == 1 else content):
                full += token
            render_json(input, input_type, full, output)
        except Exception as e:
